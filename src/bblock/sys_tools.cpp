...
void SetCharges(std::vector<double> xyz, std::vector<double> &charges, std::string mon_id, size_t n_mon, size_t nsites,
    ...
    // Alkali metal ions
        ...
        // =======>> PASTE BELOW <<=======
            ...
            for (size_t nv = 0; nv < n_mon; nv++) {
                charges[fst_ind + nv*nsites + 0] = -0.15263053737328716 * CHARGECON;
                charges[fst_ind + nv*nsites + 1] = -0.5112769383773159 * CHARGECON;
                charges[fst_ind + nv*nsites + 2] = 0.09753055744344022 * CHARGECON;
                charges[fst_ind + nv*nsites + 3] = 0.09753055744344022 * CHARGECON;
                charges[fst_ind + nv*nsites + 4] = 0.09753055744344022 * CHARGECON;
                charges[fst_ind + nv*nsites + 5] = 0.37131580342028236 * CHARGECON;
            }
        // END SECTION CHARGES
...
void SetC6LongRange(std::vector<double> &c6_lr, std::string mon_id, size_t n_mon, size_t natoms, size_t fst_ind,
    ...
    if (mon_id == "f-") {  // Fluoride
        ...
        for (size_t nv = 0; nv < n_mon; nv++) { 
            c6_lr[nv * natoms + fst_ind + 0] = 16.893691130123102; // A
            c6_lr[nv * natoms + fst_ind + 1] = 12.834239361956751; // B
            c6_lr[nv * natoms + fst_ind + 2] = 5.964486566335782; // C
            c6_lr[nv * natoms + fst_ind + 3] = 5.964486566335782; // C
            c6_lr[nv * natoms + fst_ind + 4] = 5.964486566335782; // C
            c6_lr[nv * natoms + fst_ind + 5] = 5.816201509576504; // D
        }
